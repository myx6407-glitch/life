  ...
  useEffect(() => {
    ...
    const onResults = (results: any) => {
      if (results.multiHandLandmarks && results.multiHandLandmarks.length > 0) {
        ...
        // Track hand horizontal center (using palm center/wrist as proxy)
        // MediaPipe X is normalized 0-1 (0 is left, 1 is right)
        // Since we are mirroring the video, 1-x gives us intuitive direction
      ...
      <div className="relative w-32 h-24 rounded-lg overflow-hidden border border-white/20 shadow-2xl bg-black/40 backdrop-blur-md">
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            <div className={`text-[10px] font-bold px-2 py-1 rounded bg-black/60 tracking-widest uppercase transition-all
              ${gestureStatus === 'OPEN' ? 'text-cyan-400 border border-cyan-400/50' : 
                gestureStatus === 'CLOSED' ? 'text-pink-500 border border-pink-500/50' : 'text-white/40'}`}>
